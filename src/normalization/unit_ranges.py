...
def normalize_test_row(test: Dict[str, Any], sex: str) -> Tuple[Dict[str, Any], list[str]]:
    ...
    # Scalability Strategy: Only normalize recognized tests. Others pass through.
    canonical_units = {
        "HGB": "g/dL",
        "TSH": "µIU/mL",
        "GLUCOSE": "mg/dL",
    ...
    # Only convert IF we have a strict rule for this test code
    ...
    # Fail-safe: If not in our config, TRUST THE REPORT'S extracted range
    ...
    if sex_range:
        ...
        # Only overwrite if explicitly configured to override
    ...
    # 4) Recompute flag if we have a valid range (either from config OR report)
    # Fail-safe: If report had a flag but we can't mathematically verify it, TRUST the report flag.
    ...
    # Only recompute if we actually have numbers to check against
    if low_f is not None and high_f is not None:
        new_flag = compute_flag(value_f, low_f, high_f)
        if new_flag != test.get("flag"):
            # Only override if we are sure (i.e. 'Normal' vs 'High'). 
            # If report says 'High' and we calculate 'High', great.
